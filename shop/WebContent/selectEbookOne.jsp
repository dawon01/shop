		...
		<table>
			<tr>
				...
				<td>
					...
					<div>
						분류 : <%=ebook.getCategoryName() %>
					</div>
					<div>
						지은이 : <%=ebook.getEbookAuthor() %>
					</div>
					<div>
						출판사 : <%=ebook.getEbookCompany() %>
					</div>
				</td>
			</tr>
			<tr>
				<td>
					<div>
						가격 : <%=ebook.getEbookPrice() %>
					</div>
					<div>
						상태 : <%=ebook.getEbookState() %>
					</div>
					<div>
						페이지 수 : <%=ebook.getEbookPageCount() %>
					</div>
			</tr>
			<tr>
				<td>
					줄거리 : <%=ebook.getEbookSummary() %>
				</td>			
			</tr>
		</table>
	</div>
	<div>
		<a href="<%=request.getContextPath()%>/insertCart.jsp?ebookNo=${ebookNo}">장바구니 담기</a>
	</div>
	<div>
		<h2>전자책 주문</h2>
		<%
